// add-enquiries-browser.js
// Copy and paste this entire script into your browser console while on the Enqir app
// Make sure you're logged in first!

// Realistic Indian market enquiries
const enquiries = [
  // Daily Necessities
...
// Function to add enquiries (run this in browser console)
(async function addEnquiries() {
  try {
    // Import Firebase functions
    const { collection, addDoc, serverTimestamp } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
    
    // Get db from window if available, or use global Firebase
    let db;
    if (window.db) {
      db = window.db;
    } else {
      console.error('❌ Firebase db not found. Make sure you\'re on the Enqir app page.');
      return;
    }
    ...
    console.log('🚀 Starting to add enquiries...');
    console.log(`📝 Total enquiries to add: ${enquiries.length}`);
    ...
    for (let i = 0; i < enquiries.length; i++) {
      ...
      try {
        ...
        console.log(`✅ [${i + 1}/${enquiries.length}] Added: ${enquiry.title}`);
        successCount++;
        
        // Small delay to avoid overwhelming Firestore
        await new Promise(resolve => setTimeout(resolve, 200));
      } catch (error) {
        console.error(`❌ [${i + 1}/${enquiries.length}] Error adding "${enquiry.title}":`, error);
        errorCount++;
      }
    }
    ...
    console.log(`\n🎉 Done! Check your Live Enquiries page.`);
  } catch (error) {
    console.error('❌ Script error:', error);
    console.log('\n💡 Alternative: Use the Node.js script with Firebase Admin SDK instead.');
  }
})();
