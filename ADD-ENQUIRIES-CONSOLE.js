// ADD-ENQUIRIES-CONSOLE.js
// Instructions:
// 1. Open your Enqir app in browser (localhost or live site)
...
(async function() {
  try {
    ...
    console.log(`📝 Total: ${enquiries.length} enquiries`);
    ...
    for (let i = 0; i < enquiries.length; i++) {
      ...
      try {
        ...
        console.log(`✅ [${i+1}/${enquiries.length}] ${e.title}`);
        success++;
        await new Promise(r => setTimeout(r, 200));
      } catch (err) {
        console.error(`❌ [${i+1}/${enquiries.length}] ${e.title}:`, err);
        errors++;
      }
    }
    ...
    console.log('💡 Make sure you\'re on the Enqir app page and logged in.');
  }
})();
