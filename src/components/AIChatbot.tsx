    ...
    <>
      ...
      <div className="fixed bottom-20 right-4 sm:bottom-6 sm:right-6 z-[60]">
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          ...
          <SheetContent 
            side="right" 
            className="w-[86vw] max-w-[380px] sm:w-[480px] h-[70vh] sm:h-[85vh] p-0 bg-white border-l border-gray-200 flex flex-col [&>button]:hidden"
          >
            {/* Clean Minimal Header - Compact for mobile */}
            <div className="flex items-center justify-between p-2 sm:p-4 border-b border-gray-100 bg-white">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gray-800 rounded-full flex items-center justify-center">
                  <Bot className="h-4 w-4 sm:h-5 sm:w-5 text-white" />
                </div>
                <div>
                  <h3 className="font-semibold text-gray-900 text-xs sm:text-sm">Enqir Assistant</h3>
            ...
            {/* Clean Chat Messages - Maximum space for mobile */}
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 sm:space-y-4 min-h-0 bg-gray-50/50" style={{ minHeight: 'calc(70vh - 160px)' }}>
            ...
            {/* Clean Quick Actions - Ultra compact for mobile to maximize chat space */}
            <div className="p-1.5 sm:p-4 border-t border-gray-100 bg-white">
              <div className="space-y-1 sm:space-y-3">
                <div className="flex items-center gap-1 sm:gap-2 mb-1 sm:mb-3">
                  <div className="w-3.5 h-3.5 sm:w-6 sm:h-6 bg-gray-800 rounded-full flex items-center justify-center">
                    <Sparkles className="h-1.5 w-1.5 sm:h-3 sm:w-3 text-white" />
                  </div>
                  <p className="text-[9px] sm:text-sm font-semibold text-gray-900">Quick Help</p>
                </div>
                <div className="grid grid-cols-2 gap-0.5 sm:gap-2">
                  {quickQuestions.map((question, index) => (
                    <button
                      ...
                      className="text-left p-1 sm:p-3 bg-gray-50 hover:bg-gray-100 border border-gray-200 hover:border-gray-800 rounded-md sm:rounded-xl transition-all duration-200 group disabled:opacity-50 hover:scale-105 active:scale-95"
                      disabled={isLoading}
                    >
                      <div className="flex items-center gap-0.5 sm:gap-2">
                        <div className="w-0.5 h-0.5 sm:w-1.5 sm:h-1.5 bg-gray-800 rounded-full group-hover:bg-gray-900 group-hover:scale-125 transition-all duration-200"></div>
                        <span className="text-[8px] sm:text-xs font-medium text-gray-700 group-hover:text-gray-900 truncate">{question}</span>
                        <Sparkles className="h-1.5 w-1.5 sm:h-3 sm:w-3 text-gray-800 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex-shrink-0" />
                      </div>
                    </button>
                  ))}
                </div>
            ...
            {/* Clean Input Area - Ultra compact for mobile to maximize chat space */}
            <div className="p-1.5 sm:p-4 border-t border-gray-100 bg-white">
              <div className="flex gap-1 sm:gap-3">
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full px-2 sm:px-4 py-1 sm:py-3 pr-7 sm:pr-12 text-xs sm:text-sm border border-gray-200 rounded-md sm:rounded-2xl focus:border-gray-800 focus:ring-2 focus:ring-gray-200 transition-all duration-200 bg-gray-50 focus:bg-white"
                    disabled={isLoading}
                  />
                  {inputText.trim() && (
                ...
                <button
                  ...
                  className="px-2 sm:px-4 py-1 sm:py-3 bg-gray-800 hover:bg-gray-900 disabled:bg-gray-300 text-white rounded-md sm:rounded-2xl transition-all duration-200 disabled:cursor-not-allowed flex items-center justify-center min-w-[36px] sm:min-w-[48px]"
                >
                  {isLoading ? (
              ...
              <div className="flex items-center justify-between mt-0.5 sm:mt-2 text-[8px] sm:text-xs text-gray-400">
                <span className="hidden sm:inline">Press Enter to send</span>
                <span className="flex items-center gap-0.5 sm:gap-1">
                  <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 bg-green-400 rounded-full"></div>
                  <span className="hidden sm:inline">Enqir Assistant</span>
                </span>
              </div>
            </div>
          </SheetContent>
